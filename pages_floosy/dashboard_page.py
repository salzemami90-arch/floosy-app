...
def render(month_key: str, month: str, year: int):
    ...
    if st.session_state["dash_quick_open"]:
        ...
        st.session_state.setdefault("dash_q_type", default_type if default_type in [t("مصروف", "Expense"), t("دخل", "Income")] else t("مصروف", "Expense"))
        st.session_state.setdefault("dash_q_amount", float(form_state.get("amount", 0.0)))
        st.session_state.setdefault("dash_q_category", default_category if default_category in categories else categories[-1])
        ...
        if st.session_state.get("dash_q_tax_code") not in tax_codes:
            st.session_state["dash_q_tax_code"] = default_form_tax_code if default_form_tax_code in tax_codes else (tax_codes[0] if tax_codes else "")

        st.markdown(
            """<style>
/* Make the Streamlit form behave like a centered modal */
...
/* Click outside closes (overlay is behind the modal) */
...
""",
        ...
        with st.form("quick_add_form", clear_on_submit=False):
            ...
            with r1c1:
                q_type_lbl = st.selectbox(t("النوع", "Type"), [t("مصروف", "Expense"), t("دخل", "Income")], key="dash_q_type")
            with r1c2:
            ...
            if q_selected_type == "مصروف" and tax_codes:
                q_tax_code = st.selectbox(
                    ...
                    tax_codes,
                    index=tax_codes.index(default_form_tax_code) if default_form_tax_code in tax_codes else 0,
