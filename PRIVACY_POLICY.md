...
## 1) Who we are | من نحن
GoushFi is a personal finance app for account tracking, savings, projects, and documents.

GoushFi هو تطبيق إدارة مالية شخصية لمتابعة الحساب، التوفير، المشاريع، والمستندات.
...
By default, GoushFi stores data locally on your device.
Cloud sync is optional and can be turned off in Settings.
We recommend keeping regular JSON backup exports of important data. If you want data to remain available across sessions and devices, enable cloud sync and sign in.

افتراضيًا، GoushFi يحفظ البيانات محليًا على جهازك.
المزامنة السحابية اختيارية ويمكن إيقافها من الإعدادات.
نوصي بالاحتفاظ بنسخ تصدير احتياطية منتظمة بصيغة JSON للبيانات المهمة. وإذا كان المطلوب هو استمرار توفر البيانات عبر الجلسات والأجهزة، فيرجى تفعيل المزامنة السحابية وتسجيل الدخول.

## 4) Cloud sync (optional) | المزامنة السحابية (اختيارية)
If you enable cloud sync, your app data is stored in your own cloud record through the GoushFi Supabase setup and linked to your account.
...
عند تفعيل المزامنة السحابية، يتم حفظ بيانات التطبيق في سجل سحابي خاص بك عبر إعداد Supabase الخاص بـ GoushFi ويرتبط بحسابك.
...
Service providers include Supabase for authentication/database and Streamlit Community Cloud for app hosting.
...
يشمل مزودو الخدمة Supabase للمصادقة/قاعدة البيانات وStreamlit Community Cloud لاستضافة التطبيق.
...
Please use a strong password, keep your device protected, and export backup copies of important data regularly.

نستخدم ضوابط وصول ووسائل حماية معيارية للمساعدة في حماية البيانات المحفوظة، لكن لا توجد وسيلة محلية أو سحابية مضمونة 100%.
يرجى استخدام كلمة مرور قوية، وحماية الجهاز، وتصدير نسخ احتياطية للبيانات المهمة بشكل منتظم.
...
## 12) Contact | التواصل
For privacy questions, contact: **support@goushfi.com**

للاستفسارات المتعلقة بالخصوصية، تواصل عبر: **support@goushfi.com**
